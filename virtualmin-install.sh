...
rhdeps="bind bind-chroot bind-utils caching-nameserver httpd postfix bind spamassassin procmail perl perl-DBD-Pg perl-DBD-MySQL quota iptables openssl python mailman subversion ruby rdoc ri mysql mysql-server postgresql postgresql-server rh-postgresql rh-postgresql-server logrotate webalizer php php-domxl php-gd php-imap php-mysql php-odbc php-pear php-pgsql php-snmp php-xmlrpc php-mbstring mod_perl mod_python cyrus-sasl dovecot spamassassin mod_dav_svn cyrus-sasl-gssapi mod_ssl ruby rubygems"
...
# Debian
debdeps="postfix postfix-tls postfix-pcre webmin usermin ruby libapache2-mod-ruby"
# Ubuntu (uses odd virtual packaging for some packages that are separate on Debian!)
ubudeps="postfix postfix-pcre webmin usermin ruby libapache2-mod-ruby"
